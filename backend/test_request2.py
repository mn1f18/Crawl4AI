...
LINKS_HISTORY_DIR = "link_history"  # 历史链接存储目录
USE_AI_LINK_VALIDATION = True       # 是否使用AI链接验证
COLD_START_MODE = False             # 冷启动模式：爬取所有链接并保存，不验证有效性
                                    # 设置为True时将爬取所有链接，不判断有效性，全部计入历史
...
LOG_FILE = "ai_link_decisions.jsonl"  # AI验证日志文件路径
ENABLE_LOGGING = True  # 是否启用AI验证日志

# 简化评估内容质量的函数
def evaluate_content_quality(html_content, title, url=""):
    """
    简化版评估函数，仅检查基本内容长度和有效性
    不再使用复杂评分机制，而是直接使用AI的判断
    """
    if not html_content:
        return False, "", ""
    ...
    # 简单检查内容是否有效
    is_valid = (text_length >= QUALITY_CONFIG["min_content_length"] and 
                len(paragraphs) >= QUALITY_CONFIG["min_paragraphs"])
    
    return is_valid, content_summary, content_fingerprint
                    ...
                    while retry_count <= MAX_RETRY_COUNT:
                        try:
                            ...
                            if result.success:
                                ...
                                # 评估内容质量
                                is_valid_content, content_summary, content_fingerprint = evaluate_content_quality(result.html, title, url)
                                ...
                                update_link_history(
                                    ...
                                    quality_score=ai_score,
                                    content_length=max(1, len(content)),  # 确保内容长度至少为1
                                    content_fingerprint=content_fingerprint,
                                    ai_score=ai_score,
                                    ai_reason=ai_reason,
                                    link_type=link_type
                                ...
                                # 如果内容有效，写入Excel
                                if is_valid_content and SAVE_TO_EXCEL:
                                    # 写入Excel
